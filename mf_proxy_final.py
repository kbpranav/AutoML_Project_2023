"""
===========================
Enhancing Multi-Fidelity Optimization using Proxy 
===========================
"""
...
def cnn_from_cfg(
    ...
    for cv_index, (train_idx, valid_idx) in enumerate(cv_splits, start=1):
        ...
        if model_optimizer == torch.optim.SGD:
            ...
            optimizer = model_optimizer(model.parameters(), lr=lr, weight_decay=weight_decay)
        ...
        for epoch in range(epochs):  # range(20):  # 20 epochs
            ...
            logger.info(
                f"Worker:{worker_id} => Train accuracy {train_score:.3f} | loss {train_loss} | Val Acc {1 - val_loss} ")
...
def proxy_from_cfg(
    ...
    logger.debug(f"-----Proxy Stage Ends-----")
    if optimize:
        if (1 - val_score) > 0.35:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--configspace', type=Path, default="config1.json",  # "default_configspace.json",
